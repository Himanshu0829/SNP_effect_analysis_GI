...
while read p; do
    ...
    srun --exclusive -N1 -n1 --cpus-per-task=1 bash -c "zcat ${p}_gene_ncr.vcf.gz | bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%AF\n' - > allele_freq_${p}_gene_ncr.txt" &
done < "list"
     #bcftools query -f '%CHROM\t%POS\t%REF\t%ALT\t%AF\n' your_variants.vcf > allele_frequencies.txt
